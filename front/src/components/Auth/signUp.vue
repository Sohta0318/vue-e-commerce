<template>
  ...
  <form @submit.prevent="createUser">
    ...
    <button value="Sign Up" title="Submit form" class="icon-arrow-right">
      <span>Sign up</span>
    </button>
  </form>
  <div class="text">
    <p @click="signUpOrLogin">Already have an account? Log in</p>
    <h2 v-if="failSignUp" class="fail">You already have an account!</h2>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    signUpOrLogin() {
      this.$store.dispatch("signUpOrLogin");
    },
  },
  computed: {
    failSignUp() {
      return this.$store.getters.trySignUp;
    },
  },
};
</script>
