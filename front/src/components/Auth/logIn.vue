<template>
  <!-- action="http://lab.alexcican.com/minimal_signup_form/about.html"
    method="get" -->
  <form @submit.prevent="logIn">
    ...
    <button value="Log In" title="Submit form" class="icon-arrow-right">
      <span>Log In</span>
    </button>
  </form>
  <div class="text">
    <h2 v-if="failLogin" class="fail">Couldn't find any match account</h2>
    <p @click="signUpOrLogin">Don't have an account? Sign Up here</p>
    <p>Forget password?</p>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    signUpOrLogin() {
      return this.$store.dispatch("signUpOrLogin");
    },
  },
  computed: {
    failLogin() {
      return this.$store.getters.tryLogin;
    },
  },
};
</script>
